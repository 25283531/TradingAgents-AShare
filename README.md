...
资金流向和龙虎榜采用多源降级：AKShare 的东财接口、同花顺接口、雪球接口及同花顺/东财排名接口会依次尝试；接口返回空值或“暂不可用”时，路由层继续尝试其他供应商。可选配置 `TA_TUSHARE_TOKEN` 后还可加入 Tushare Pro 的 `moneyflow`、`龙虎榜` 数据作为补充（需遵守其免费额度和授权）。所有结果会标注来源；当所有来源失败时，分析明确标记数据不可用并触发不交易约束，不会用估算值填充。

### A 股短线决策与自我进化

短线流程以 2～5 个交易日为评估周期，重点观察市场情绪、行业轮动、热钱流向、相对强度、流动性和量化陷阱；基本面用于识别重大风险与催化因素，不单独决定短线涨跌。代码硬规则先执行，研究总监和风控仍可否决候选。

每次短线预测都会写入独立 SQLite 学习库（默认 `data/learning.db`），保存预测方向、因子证据、命中结果和错误类型。API 启动后自动复盘已到期预测：严格使用中国交易所日历和未复权日线收盘价，缺少日历、停牌或行情延迟时保持待复盘，不补造价格。观望和不交易不会被计算为交易收益，命中率是“预测方向命中率”。

学习流程参考 [Hermes](https://github.com/NousResearch/hermes-agent) 的持久经验、检索和经验提炼思路：历史经验只在预测日期之前检索，达到最小样本后以时间切分验证集调整因子权重，策略版本和拒绝原因均可审计。系统不会修改模型代码、调用交易接口或凭价格异动断言量化资金身份。学习库目录应作为 Docker 持久卷；可用 `TA_LEARNING_DB`、`TA_LEARNING_ENABLED` 和 `TA_LEARNING_REVIEW_WORKER` 配置路径、开关及复盘 worker。
